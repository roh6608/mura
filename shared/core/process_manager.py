...
class ProcessManager[ConfigT](ABC):
    ...
    Each application subclasses this and implements ``_create_workers`` to
    build its worker set e.g. a system monitor plus  a compute
    worker (in the future multiple compute workers). ``config`` is the application's
    runtime-loaded dataclass, stored for workers to read, never write.

    ``_create_workers`` is called at the end of ``__init__``, so subclasses
    that need extra constructor state must set it before calling
    ``super().__init__``.
    """
